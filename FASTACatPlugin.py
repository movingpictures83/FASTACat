...
def clean_fasta_records(fa, annotation_df, outputFASTAFile, outputAssemblyAnnotation):
...
class FASTACatPlugin:
    def input(self, infile):
        self.parameters = PyIO.readParameters(infile)
        self.input_dir = PyPluMA.prefix()+"/"+self.parameters["inputdir"]
        self.db_version = self.parameters["dbversion"]
        self.annotations_dir = PyPluMA.prefix()+"/"+self.parameters["annotationsdir"]
    def run(self):
        pass
    def output(self, outfile):
        ...
        ensemblDatabaseVersion  = self.db_version
        ...
        for fasta in fasta_list:
            clean_fasta_records(fasta, annotation_df, outputFASTAFile, outputAssemblyAnnotation)
